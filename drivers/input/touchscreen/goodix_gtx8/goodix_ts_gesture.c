	...
	struct goodix_ext_module *module)
{
	...
	if (core_data->fod_status || core_data->tid->aod_status) {
		if ((FP_Event_Gesture == 1) && (temp_data[2] == 0x46)) {

			x = temp_data[4] | (temp_data[5] << 8);
			y = temp_data[6] | (temp_data[7] << 8);
			overlapping_area = temp_data[8] * 100 / 9;

			if (overlapping_area > 100) {
				overlapping_area = 100;
			}

			area = temp_data[9];
			/*conversion 8bit complete code to origin code*/
			cplt_angle = temp_data[12] & 0xFF;
			angle = ~(cplt_angle & 0x7F) + 1;
			angle = cplt_angle & 0x80 ? -angle : angle;
			long_axis = temp_data[10]*72/10;
			short_axis = temp_data[11]*72/10;
			...
			input_report_abs(core_data->input_dev, ABS_MT_TOUCH_MAJOR, long_axis);
			input_report_abs(core_data->input_dev, ABS_MT_PRESSURE, 50);
			input_report_abs(core_data->input_dev, ABS_MT_WIDTH_MAJOR, 100);
			input_report_abs(core_data->input_dev, ABS_MT_WIDTH_MINOR, 100);

			core_data->fod_pressed = true;
			__set_bit(0, &core_data->touch_id);

			ts_debug("Gesture report, x=%d, y=%d, overlapping_area=%d, area=%d, angle%d, long_axis=%d, short_axis=%d",
					x, y, overlapping_area, area, angle, long_axis, short_axis);
...
re_send_ges_cmd:
	if (core_data->double_wakeup && (core_data->fod_status || aod_status)) {
	...
	} else if (core_data->double_wakeup && !core_data->fod_status && !aod_status){
		state_data[0] = GSX_GESTURE_CMD;
		state_data[1] = 0x03;
		state_data[2] = 0xF5;
		ret = goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		...
		ts_info("Set IC double wakeupFOD fail: double_wakeup = %d, \
			fod_status = %d, aod_status = %d",				\
			core_data->double_wakeup, core_data->fod_status, aod_status);
	}
	...
	struct goodix_ext_module *module)
{
	...
	if (core_data->double_wakeup && (core_data->fod_status || aod_status)) {
		...
		ts_info("Get IC mode falied,core_data->double_wakeup=%d,core_data->fod_status=%d,core_data->aod_status=%d;",
			core_data->double_wakeup, core_data->fod_status, aod_status);
	}
	if (!(core_data->gesture_enabled || aod_status))
		return EVT_CONTINUE;
	...
	if (ret != 0) {
		...
		ts_debug("Set IC in doze mode---enable irq wake \n");
...
{
	int ret=0;
	ts_debug("Set IC in doze mode---disable_irq_wake \n");
